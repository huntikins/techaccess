  ...
  <v-layout column justify-center align-center>
    <v-flex xs12 sm8 md6>
      ...
      <v-card class="transparent" flat>
        <v-card-title class="headline text-center d-block pb-0 mb-0" v-html="heading" style="word-break: normal;">
        </v-card-title>
        <v-card-text class="text-center pt-0 mt-0" v-html="subheading">
        </v-card-text>
        <v-row class="pa-4">
          <template v-for="(value, name, index) in iconblock">
          <v-col :md="3" :sm="6" :key="value.id" >
            <v-card hover v-ripple class="pa-4">
                <v-img width="50%" contain :src="value.icon" class="mx-auto mt-4"></v-img>
              <v-card-subtitle
                class="text-center white--text"
              >{{value.text}}</v-card-subtitle>
            </v-card>
          </v-col>
          </template>
        </v-row>
